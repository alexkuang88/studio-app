    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            {groupBy === "month" ? "按月" : "按日"}
            {dateFrom || dateTo ? ` | ${dateFrom || "最早"} — ${dateTo || "最新"}` : " | 全部时间"}
            {" | 共 "}{data?.total_orders || 0} 单
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <div className="flex items-center gap-2 mb-2"><ShoppingCart size={18} className="text-purple-500" /><span className="text-xs text-gray-500">订单总数</span></div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <div className="flex items-center gap-2 mb-2"><BarChart3 size={18} className="text-blue-500" /><span className="text-xs text-gray-500">订单金额</span></div>
          <div className="text-2xl font-bold text-blue-600">{loading ? "—" : `${((data?.total_amount || 0) + (data?.pending_amount || 0)).toLocaleString("zh-CN")} 万`}</div>
          <div className="text-xs text-gray-400 mt-1">已完成 {(data?.total_amount || 0).toLocaleString("zh-CN")} 万</div>
        </div>
        <div className="bg-green-50 rounded-xl border border-green-200 p-4">
          <div className="flex items-center gap-2 mb-2"><DollarSign size={18} className="text-green-500" /><span className="text-xs text-green-700 font-medium">已确认收入</span></div>
          <div className="text-2xl font-bold text-green-700">{loading ? "—" : `¥ ${(data?.total_revenue || 0).toLocaleString("zh-CN")}`}</div>
          <div className="text-xs text-green-500 mt-1">{data?.completed_count || 0} 单已完成</div>
        </div>
        <div className="bg-yellow-50 rounded-xl border border-yellow-200 p-4">
          <div className="flex items-center gap-2 mb-2"><Clock size={18} className="text-yellow-500" /><span className="text-xs text-yellow-700 font-medium">待完成收入</span></div>
          <div className="text-2xl font-bold text-yellow-700">{loading ? "—" : `¥ ${(data?.pending_revenue || 0).toLocaleString("zh-CN")}`}</div>
          <div className="text-xs text-yellow-500 mt-1">{data?.pending_count || 0} 单进行中</div>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <div className="flex items-center gap-2 mb-2"><span className="text-lg">💰</span><span className="text-xs text-gray-500">潜在收入</span></div>
          <div className="text-2xl font-bold text-gray-800">{loading ? "—" : `¥ ${((data?.total_revenue || 0) + (data?.pending_revenue || 0)).toLocaleString("zh-CN")}`}</div>
              ...
              {loading ? (
                ...
                (data?.periods || []).map((row: any) => (
                  <tr key={row.period} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-right font-mono text-green-600 font-medium">¥ {row.total_revenue?.toLocaleString("zh-CN")}</td>
                    <td className="px-4 py-3 text-right font-mono text-yellow-600">¥ {row.pending_revenue?.toLocaleString("zh-CN")}</td>
                  </tr>
                ))
              )}
            </tbody>
